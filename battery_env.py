...
class BatteryEnv(gym.Env):
    ...
    def step(self, action):
        ...
        #logger.debug(f"SOC: {self.soc}, Done: {done}")
    ...
    def _calculate_reward(self): #v_batt_values
        # SOC Variance Reward
        soc_variance = np.var(self.soc)
        normalized_soc_variance = soc_variance / np.max(self.soc)  # Normalize SOC variance
        r_soc = -np.log(normalized_soc_variance + 1e-8)  # Use normalized SOC variance

        # Safeguard to avoid division by zero or overly large rewards
        temperature_variance = np.var(self.t_cell)
        mean_temp = np.mean(self.t_cell)
        normalized_temp_variance = max(temperature_variance / (mean_temp + 1e-8), 1e-8)  # Avoid division by 0
        r_temp = -np.log(normalized_temp_variance + 1e-8)  # Log normalization

        # Usable Capacity Reward
        avg_soc = np.mean(self.soc)
        temp_deviation = np.mean(np.abs(self.t_cell - 298.15))  # Average deviation from 25°C
        usable_capacity_reward = avg_soc * np.exp(-temp_deviation / 10)  # Penalize high temp deviations

        # Switching Penalty
        switch_penalty = np.sum(np.abs(np.diff(self.switch_state))) / len(self.switch_state)
        switch_penalty = np.power(switch_penalty, 2)
        r_switch = -self.alpha * switch_penalty

        # Discharge Penalty
        discharge_penalty = -np.sum(np.maximum(self.soc - 0.8, 0))
        r_switch += 0.5 * discharge_penalty
        ...
        soc_variance = np.var(self.soc)  # Calculate SOC variance
        normalized_variance = soc_variance / (np.mean(self.soc) + 1e-8)  # Avoid division by zero
        r_balance = -np.log(normalized_variance + 1e-8)  # Penalize high normalized variance

        #Fifnal Reward
        reward = 0.8 * r_soc + 0.2 * r_temp + r_switch + penalty + usable_capacity_reward + r_balance
    ...
    def render(self, mode='human'):
        print(f"Reward: {reward}, SOC: {self.soc}, Temp: {self.t_cell}, Switch State: {self.switch_state}, Voltage: {self.voltage}, Usable Capacity Reward: {usable_capacity_reward}")