...
class BatteryEnv(gym.Env):
    def __init__(self):
        ...
        self.total_episodes = 50_000  # Total planned episodes
    ...
    def step(self, action):
        ...
        _, _, _, _, _, _, reward, done = self.calculate_reward()  # Unpack only what is needed
    ...
    def reset(self, seed=None, options=None):
        ...
        self.voltage = (np.full(self.num_cells, 3.6) - 3.3) / (4.1 - 3.3)
        # Normalize other state variables if applicable
        self.parameters = self.compute_dynamic_parameters_vectorized(
        self.soc * (0.6 - 0.4) + 0.4)  # Restore SOC to the true range
    ...
    def _get_obs(self):
        normalized_soc = (self.soc - 0.1) / (0.9 - 0.1)
        normalized_temp = (self.t_cell - 290) / (350 - 290)
        normalized_voltage = (self.voltage - 3.3) / (4.1 - 3.3)
        return np.column_stack((normalized_soc, self.switch_state, normalized_temp, normalized_voltage))
